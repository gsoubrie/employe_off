...
// Types qui utilisent le format de sujet Payfit
...
function buildPayfitSubject(leave) {
  ...
  return `${leave.typeLabel};${leave.employeeLastName};${leave.debut};${leave.periodeDebut};${leave.fin};${leave.periodeFin} => ${nb} j`;
}
...
async function submitLeave(formData) {
  ...
  // On stocke les IDs de tous les managers pour la vue manager
  ...
  const docData = {
    ...
    // On garde managerId pour compatibilité mais on utilise managerIds pour les mails
...
// ── Email demande (vers les 3 managers) ──────────────────────
...
function sendLeaveRequestEmail(leave) {
  ...
  // Premier manager en destinataire principal, les autres en CC
...
${window.location.origin}${window.location.pathname.replace("index.html", "")}manager.html
